...
async function loadReplayData() {
  ...
  startFrame = Number(recording.start_frame || 0);
  endFrameExclusive = Number(recording.end_frame_exclusive || startFrame + Number(recording.captured_frames || 1));
  frame = startFrame;
  elements.frameSlider.min = String(startFrame);
  elements.frameSlider.max = String(endFrameExclusive - 1);
  elements.frameSlider.value = String(frame);
  elements.frameReadout.textContent = `${frame} / ${endFrameExclusive - 1}`;
...
function renderSummary() {
  ...
  elements.summaryLabel.textContent = classification === "global_time_shift"
    ? "global_time_shift"
    : classification;
...
async function computeOffsetSearch(region) {
  const offsets = [];
  const rangeStart = Number(recording?.start_frame || 0);
  const rangeEnd = Number(recording?.end_frame_exclusive || rangeStart + Number(recording?.captured_frames || 0));
  ...
  for (let offset = -temporalWindow; offset <= temporalWindow; offset += 1) {
    ...
    for (let referenceIndex = rangeStart; referenceIndex < rangeEnd; referenceIndex += 1) {
...
function renderFrame() {
  ...
  const maxFrame = endFrameExclusive - 1;
  frame = Math.max(startFrame, Math.min(frame, maxFrame));
  const fileName = formatFrameName(frame);
  const data = recording.per_frame?.find((item) => item.frame === frame) || {};
...
function clampFrame(index) {
  const minFrame = Number(recording?.start_frame || 0);
  const maxFrame = Number(recording?.end_frame_exclusive || minFrame + Number(recording?.captured_frames || 1)) - 1;
  return Math.max(minFrame, Math.min(Number(index), maxFrame));
}
...
function setPlaying(nextPlaying) {
  ...
  timer = setInterval(() => {
    const minFrame = Number(recording?.start_frame || 0);
    const maxFrame = Number(recording?.end_frame_exclusive || minFrame + Number(recording?.captured_frames || 1)) - 1;
    frame = frame >= maxFrame ? minFrame : frame + 1;
    renderFrame();
  }, 1000 / targetFps);
}
