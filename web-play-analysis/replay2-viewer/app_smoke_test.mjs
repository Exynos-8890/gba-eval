...
for (const file of expectedFiles) {
  if (!existsSync(new URL(file, root))) {
    throw new Error(`missing expected replay2 viewer file: ${file}`);
  }
}
...
for (const obsolete of [
  ...
  if (html.includes(obsolete) || js.includes(obsolete) || server.includes(obsolete) || css.includes(obsolete)) {
    throw new Error(`obsolete replay1 token is still present: ${obsolete}`);
  }
}

console.log("replay2 viewer smoke test passed");
